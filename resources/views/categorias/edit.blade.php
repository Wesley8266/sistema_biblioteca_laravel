...
<body class="bg-[#0F0A0A]">
    ...
    <!-- Sidebar -->
    <aside class="w-64 h-screen bg-[#1A1212] text-[#EDE6DC] fixed left-0 top-0 flex flex-col border-r border-[#312322]">
        ...
        <div class="p-4 border-t border-[#312322]">
            <form method="POST" action="{{ route('logout') }}">
                @csrf

                <button
                    type="submit"
                    class="w-full bg-[#BFA76F] text-black font-semibold py-3 rounded-xl hover:bg-[#FF9500] transition">
                    Sair
                </button>
            </form>
        </div>
    ...
    <main class="ml-64 min-h-screen flex justify-center items-center p-8">
        ...
        <div class="w-full max-w-2xl bg-[#171111] border border-[#312322] rounded-3xl shadow-2xl p-8">
            ...
            <p class="text-[#A89B95] mb-8">
                Cadastre uma nova categoria para organizar os livros da biblioteca.
            </p>

            <form action="{{ route('categorias.update', $category->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div>
                    <label
                    ...
                    <input
                        ...
                        required
                        placeholder="Ficção científica, romance, suspense, terror..."
